...
function PDU(command, options) {
    ...
    const decode = options !== false // default true
    return this.fromBuffer(command, decode)
  }
	options = options || {};
...
PDU.fromStream = function(stream) {
  ...
  if (pdu) {
    pdu.raw = buffer.toString('hex')
  }

  return pdu
};

PDU.fromBuffer = function(buffer, decode) {
	if (buffer.length < 16 || buffer.length < buffer.readUInt32BE(0)) {
		return false;
	}
	return new PDU(buffer, decode);
};
...
PDU.prototype.fromBuffer = function(buffer, decode) {
