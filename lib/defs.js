...
encodings.IA5 = {
  // GSM 03.38 7bit
  match: function (value) {
    return value === this.decode(this.encode(value))
  },
  encode: function (value, udhi = 0) {
    let encodedBuf = encodings.ASCII.encode(value)

    if (udhi === 64) {
      const udhBuffer = Buffer.from('0000000')
      encodedBuf = Buffer.concat([udhBuffer, encodedBuf])
    }
    // We should create septeps now.
    const octets = []
    for (const b of encodedBuf) {
      octets.push(('0000000' + b.toString(2)).slice(-7))
    }

    for (let i in octets) {
      i = parseInt(i)
      const freeSpace = 8 - octets[i].length
    ...
    for (const octet of octets) {
      if (octet.length > 0) {
        hex += ('00' + parseInt(octet, 2).toString(16)).slice(-2)
      }
    }
    return Buffer.from(hex, 'hex')
  },
  decode: function (value, udhi = 0) {
    if (!Buffer.isBuffer(value)) {
      value = Buffer.from(value)
    }
    if (udhi === 64) {
      value = Buffer.concat([Buffer.from('000000'), value])
    }

    const code = value.toString('hex')
    // We are getting 'septeps'. We should decode them.
    let binary = ''
    for (let i = 0; i < code.length; i++) {
      binary += ('0000' + parseInt(code.slice(i, i + 1), 16).toString(2)).slice(
        -4
      )
    }
    ...
    let cursor = 0
    let fromPrevious = ''
    while (binary.length > cursor) {
      const remaining = 7 - fromPrevious.length
      const toNext = 8 - remaining
      const septep =
        binary.slice(cursor + toNext, cursor + toNext + remaining) +
        fromPrevious
      charCodes.push(parseInt(septep, 2))

      fromPrevious = binary.slice(cursor, cursor + toNext)
      if (toNext === 8) {
        fromPrevious = ''
      } else {
        cursor += 8
    ...
    if (udhi === 64) {
      charCodes = charCodes.slice(7)
    }
    return encodings.ASCII.decode(charCodes)
  }
}
...
filters.message = {
	...
	decode: function(value) {
		...
		if (encodings[encoding]) {
			result.message = encodings[encoding].decode(result.message, udhi);
		}
		return result;
	}
};
